...
const main = async () => {
  console.log('******/  IN Todo Task FUNCTION /******');
  ...
  // Get Todo list fron localdb
  const proofs_list_object = await db.getAllProofs();

  // Use the node's keypair to sign the Todo list
  ...
  // upload the proofs of the Todo on web3.storage
  const path = `./Todo/proofs.json`;
  ...
  if (storageClient) {
    ...
    console.log('User Todos proof uploaded to IPFS: ', proof_cid);
