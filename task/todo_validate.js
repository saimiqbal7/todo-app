...
module.exports = async (submission_value, round) => {
  console.log('******/ Todo CID VALIDATION Task FUNCTION /******');
  ...
  // check each item in the Todos list and verify that the node is holding that payload, and the signature matches
  let isTodo = await verifyTodos(output.proofs);
  console.log('IS Todo True?', isTodo);

  if (isNode && isTodo) return true; // if both are true, return true
  else return false; // if one of them is false, return false
};

// verify the Todo signature by querying the other node to get it's copy of the Todo
async function verifyTodos(proofs_list_object) {
  ...
  for (const proofs of proofs_list_object) {
    ...
    // verify the signature of the todo for each nodes
    for (const nodeUrl of nodeUrlList) {
      console.log('cheking todo on ', nodeUrl);

      // get all todo in this node
      const res = await axios.get(
        `${url}/task/${TASK_ID}/todo/get/${publicKey}`,
      );

      // TEST hardcode the node endpoint
      // const res = await axios.get(`${nodeUrl}/todo/get/${publicKey}`);
      ...
      if (AuthUserList.hasOwnProperty(todo.publicKey)) {
        ...
        if (todo.publicKey.length == 42) {
          ...
          const messageUint8Array = new Uint8Array(
            Buffer.from(JSON.stringify(todo.data)),
          );
          const signature = todo.signature;
          const publicKey = todo.publicKey;
